...
int main()
{
    ...
    printf("Enter a string to find words ending with 's' -\n");
    ...
    if ( length > 0)
    {
        ...
        printf("The words ending with s are - \n");
...
//@ Finding Words ending with 's' and storing them in a 2D Char Array
void storeIn2DArray(char *str, int length, int *words, char (*words2DArray)[length])
{
    int i, j, k, l;
    ...
    for ( i = 0, k = 0 ; str[i] != '\0' ; i++)
    {
        ...
        j = i;      //$ Preserving value of i in j
        while ( str[i] != ' ' && str[i] != '\0' )  //$ Finding Next Space char
            i++;

        
        if (str[i-1] == 's' || (str[i-1] == '.' && str[i-2] == 's' ))    //$ Checking for 's' char if word ends
        {
            for( l = 0 ; j < i && str[j] != '.' ; l++, j++ )
            {
                words2DArray[k][l] = str[j]; //$ Storing words ending with 's' in 2D array
            }
            words2DArray[k][l] = '\0';
            k++;
        }
